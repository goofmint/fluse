...
- [x] `docs/e2e-checklist.md` に手動シナリオを記載
- [x] シナリオ: 初回 `init` → `start` → QRスキャン → Dart変更 → asset変更 → コンパイルエラー → 復旧 → IDE 等からの Hot Restart → 接続が維持されることの確認（設計 §10-6）
- [x] シナリオ: adb 不在時の APK 配信導線（動く経路は `fluse start` 中の `/apk` のみ。`init` / `rebuild` からの配信は未配線として制約に明記）
- [x] シナリオ: 署名衝突の3択（2 を選んだ後の別 ID ビルドが未対応である点も制約に明記）
- [x] シナリオ: `pubspec.lock` 変更 → `APP_OUTDATED` → `rebuild`
- **完了条件**: 全シナリオが実機で成功する
  - シナリオの記載は完了。**実機での実行は未実施**（チェックリストの「実施結果」を埋めるところまでが残り）
- **依存**: Task 5.10, Task 4.6, **Task 4.5**（「コンパイルエラー → 復旧」シナリオがエラーオーバーレイの表示・解除を検証対象にするため）
- **推定時間**: 6h

#### Task 6.4: 性能計測と最適化

- [x] 反映経路の各段（compile / PUT / reload / reassemble）の所要時間を計測（CI のデスクトップ経路。実機は未実施）
- [x] 設計 §8.1 の目標との比較レポートを作成（`.tmp/perf-result.md`）
- [x] 目標未達の箇所を特定し最適化する（未達なし。§8.2 の最適化は実装済みで、緩めていない）
- **完了条件**: 1ファイル変更 → 画面反映が < 1.0秒。未達の場合は原因と対策を文書化
  - デスクトップ経路で p95 221ms（目標 1.0秒）。**実機（トンネル + Runtime 経由）は未計測**で、手順を `.tmp/perf-result.md` に残した
- **依存**: Task 6.3
- **推定時間**: 4h
